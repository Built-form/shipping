-- Shipments entity: the three tables (was cold-start DDL in
-- src/services/shipment-sync.js, and src/db/migrations/2026-09-18_shipments_tables.sql).
--
...
-- CLOSED or CANCELLED. Through rollout step 3 it is a derived, rebuildable
-- shadow of the legacy columns and tables (orders.container_number, the draft
-- and planned allocation tables, draft_containers), kept in step by fail-soft
-- hooks in src/handlers/orders.js and src/services/shipment-sync.js. The
-- tables are inert until tools/backfill-shipments.js arms them.
--
